...
if (isset($_POST['ubahFinish'])) {
    $code = $_POST['code'];
    $id = $_POST['id_pembelian_product'];
    $update_approved = mysqli_query($conn, "UPDATE pembelian_product SET action = 'Finish' WHERE id_pembelian_product = '$id'");
    if ($update_approved) {
        header("location: ./pembelian?id=$code");
    }
}
...
if (isset($_POST['ubahApproved'])) {
    ...
    $id = $_POST['id_pembelian_product'];
    $date = $_POST['date'];
    $update_approved = mysqli_query($conn, "UPDATE pembelian_product SET tgl_kehadiran = '$date', action = 'Approved' WHERE id_pembelian_product = '$id'");
    if ($update_approved) {
        header("location: ./pembelian?id=$code");
    }
}
...
document.location.href = './service_produk'
        ...
        if (move_uploaded_file($image['tmp_name'], $folder)) {
            $date = date('Y-m-d h:i:s');
            $update_service_list = mysqli_query($conn, "UPDATE product SET nama_product = '$nm_service', harga_product =
'$hrg_service', desk_product = '$desk_service', image_product = '$img', tanggal_upload = '$date' WHERE id_product =
'{$data_edit_service['id_product']}' ");
...
document.location.href = './service_produk'
        ...
        $date = date('Y-m-d h:i:s');
        $update_service_list = mysqli_query($conn, "UPDATE product SET nama_product = '$nm_service', harga_product =
'$hrg_service', desk_product = '$desk_service', tanggal_upload = '$date' WHERE id_product =
'{$data_edit_service['id_product']}' ");
        if ($update_service_list) {
            echo "<script>
alert('Data Berhasil Diubah Tanpa Gambar');
...
if (isset($_POST['save_on'])) {
    ...
    if ($view_change_on) {
        ...
        if ($update_change_on) {
            echo "<script>
alert('Kondisi Berhasil Menjadi On!');
document.location.href = './data_pelanggan'
</script>";
        } else {
            echo "<script>
alert('Kondisi Tidak Berhasil Menjadi On!');
document.location.href = './data_pelanggan'
</script>";
        }
    }
...
if (isset($_POST['save_off'])) {
    ...
    if ($view_change_off) {
        ...
        if ($update_change_off) {
            echo "<script>
alert('Kondisi Berhasil Menjadi Off!');
document.location.href = './data_pelanggan'
</script>";
        } else {
            echo "<script>
alert('Kondisi Tidak Berhasil Menjadi Off!');
document.location.href = './data_pelanggan'
</script>";
        }
    }
...
document.location.href = './service_produk'
        ...
        if (move_uploaded_file($image['tmp_name'], $folder)) {
            $role = "Product";
            $insert_product = mysqli_query($conn, "INSERT INTO product (code_product, nama_product, jumlah_product, harga_product,
desk_product, image_product, status_product, tanggal_upload, status) VAlUES ('$code_gabung','$nm_produk', '$jml_produk',
'$hrg_produk', '$desk_produk', '$img', '$status', '$date', '$role') ");
            if ($insert_product) {
                echo "<script>
...
document.location.href = './service_produk'
                ...
                echo "<script>
alert('Data Tidak Dapat Di Tambah!');
document.location.href = './service_produk'
</script>";
            }
        }
        ...
        $role = "Product";
        $insert_product = mysqli_query($conn, "INSERT INTO product (code_product, nama_product, jumlah_product, harga_product,
desk_product, status_product, tanggal_upload, status) VAlUES ('$code_gabung','$nm_produk', '$jml_produk', '$hrg_produk',
'$desk_produk', '$status', '$date', '$role') ");
        if ($insert_product) {
            echo "<script>
alert('Data Berhasil Di Tambahkan Tanpa Gambar');
document.location.href = './service_produk'
</script>";
        } else {
            echo "<script>
...
document.location.href = './service_produk'
        ...
        if (move_uploaded_file($image['tmp_name'], $folder)) {
            ...
            $role = "Service";
            $insert_service = mysqli_query($conn, "INSERT INTO product (code_product, nama_product, harga_product, desk_product,
image_product, status_product, tanggal_upload, status) VALUES ('$code_gabung', '$nama', '$harga', '$desk', '$img',
'$status_service', '$tgl_upload', '$role') ");
            if ($insert_service) {
                echo "<script>
alert('Anda Berhasil Mengupload List Service Dengan Image.');
document.location.href = './service_produk'
</script>";
            } else {
                echo "<script>
alert('Data Tidak Berhasil Dibuat!');
document.location.href = './service_produk'
</script>";
            }
        }
        ...
        $role = "Service";
        $insert_service = mysqli_query($conn, "INSERT INTO product (code_product, nama_product, harga_product, desk_product,
status_product, tanggal_upload, status) VALUES ('$code_gabung','$nama', '$harga', '$desk', '$status_service',
'$tgl_upload', '$role') ");
        if ($insert_service) {
            echo "<script>
alert('Anda Berhasil Mengupload List Service Tanpa Image.');
document.location.href = './service_produk'
</script>";
        } else {
            echo "<script>
...
if (isset($_POST['pass_change'])) {
    ...
    if (mysqli_num_rows($view_chg) === 1) {
        ...
        $update_chg = mysqli_query($conn, "UPDATE user SET password = '$pass_chg', konfirmasi = '$konfir_chg' WHERE username =
'{$row['username']}' ");
        if ($update_chg) {
            echo "<script>
alert('Anda Berhasil Mengganti Kata Sandi.');
document.location.href = './log-in'
</script>";
        } else {
            echo "<script>
alert('Anda Tidak Berhasil Mengganti Kata Sandi.');
document.location.href = './log-in'
</script>";
        }
    } else {
...
if (isset($_POST['masuk'])) {
    ...
    $view_login = mysqli_query($conn, "SELECT * FROM user WHERE username = '{$username_login}' AND password =
'{$password_login}' AND status = 'ON' ");
    if (mysqli_num_rows($view_login) == 0) {
        echo "<script>
alert('Email & Password Anda Masukkan Salah! Atau Status Akun Anda OFF.');
document.location.href = './log-in'
</script>";
    } else {
...
document.location.href = './'
                ...
                echo "<script>
alert('Anda Tidak Berhasil Masuk.');
document.location.href = './log-in'
</script>";
            }
        } else {
